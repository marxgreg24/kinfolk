    ...
    <div className="flex min-h-screen" style={{ background: '#fafaf8' }}>
      ...
      <div className="flex-1 flex flex-col ml-64">
        <main className="flex-1 p-8">
          ...
          {/* Header */}
          <div className="mb-7">
            <p className="text-xs font-merriweather tracking-[0.25em] text-secondary uppercase mb-1">My Clan</p>
            ...
            <p className="text-gray-400 text-sm mt-1 font-merriweather">
              Define your relationship to each clan member below.
            </p>
          </div>
          ...
          {!membersLoading && otherMembers.length > 0 && (
            <div className="bg-white border border-gray-100 rounded-2xl px-6 py-4 mb-6 flex items-center gap-6 shadow-sm">
              <div>
                <p className="text-2xl font-bold text-primary font-merriweather">{otherMembers.length}</p>
                <p className="text-xs text-gray-400 font-merriweather uppercase tracking-wider mt-0.5">Clan Members</p>
              </div>
              <div className="h-8 w-px bg-gray-100" />
              <div>
                <p className="text-2xl font-bold text-secondary font-merriweather">
                  {Object.keys(relMap).length}
                </p>
          ...
          {!membersLoading && otherMembers.length === 0 && (
            <div className="bg-white border border-gray-100 rounded-2xl p-16 text-center shadow-sm">
              ...
              <p className="text-gray-500 font-merriweather text-sm">No other clan members yet.</p>
              <p className="text-gray-400 font-merriweather text-xs mt-1">Your clan leader will add members soon.</p>
            </div>
          )}
          ...
          {!membersLoading && otherMembers.length > 0 && (
            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
              ...
              {otherMembers.map((member, i) => (
                <div
                  key={member.id}
                  className={`flex items-center justify-between px-6 py-4 gap-4 hover:bg-gray-50/70 transition-colors ${i < otherMembers.length - 1 ? 'border-b border-gray-50' : ''}`}
                >
                  ...
                  <RelationshipSelector memberId={member.id} clanId={clanId} existingRelationship={relMap[member.id]} />
                </div>
              ))}
            </div>
          )}
